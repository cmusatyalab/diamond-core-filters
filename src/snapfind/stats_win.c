...
void set_summary_attr(GtkWidget *widget) {
...
init_filt_data(filt_data_t *fdata)
{
    ...
    gtk_box_pack_start (GTK_BOX(fdata->box), 
			fdata->sbox3, FALSE, FALSE, 0);
    ...
    gtk_box_pack_start(GTK_BOX(fdata->sbox3), fdata->drop_val, FALSE, FALSE, 0);
...
get_dev_name(ls_search_handle_t shandle, ls_dev_handle_t dev_handle)
{
	...
	hent = gethostbyaddr((char *)&dchar.dc_devid, sizeof(dchar.dc_devid), AF_INET);
	if (hent == NULL) {
		/* XXX debug */
...
void
new_dev_status(ls_search_handle_t shandle, int dev, ls_dev_handle_t dev_handle)
{
...
update_dev_status(int dev, dev_stats_t *dstats, int verbose)
{
	...
	if (dstats->ds_objs_total == 0) {
			...
			(double)dstats->ds_objs_nproc)/ (double)dstats->ds_objs_total;
		if (!(done <= 1.0)) {
			done = 1.0;
		}
	}
...
setup_table(dev_stats_t *dstats, int num_dev, int verbose)
{
    ...
    page_master.pm_notebook = gtk_notebook_new();
   	...
   	gtk_box_pack_start(GTK_BOX(box), page_master.pm_notebook, FALSE, FALSE, 0);
	gtk_widget_set_name(label, "heading");
    if (verbose) {
...
stats_main(void *data)
{
	...
	/* only show the stats window after we add all the components,
	 * so it gets the right size */
	...
	while(1) {
		...
		for (i = 0; i < num_dev; i++) {
			len = DEV_STATS_SIZE(MAX_FILT);
			err = ls_get_dev_stats(shandle, dev_list[i], 
					dstats, &len);
			//dump_stats(dstats);
			if (err) {
...
create_stats_win(ls_search_handle_t shandle, int verbose)
{
	 ...
	 */
	thread_close = 0;
	err = pthread_create(&stats_thread, PATTR_DEFAULT, stats_main, (void *)shandle);
	if (err) {
