			...
			lf_obj_handle_t * ohandles, int numarg, void *fdata)
{
  ...
  /* save some attribs */
  lf_write_attr(fhandle, ohandle, IMG_HEADERLEN, sizeof(int), (char *)&headerlen);
  ...
  lf_write_attr(fhandle, ohandle, RGBA_IPL_IMAGE, sizeof(char*), (char *)img);
			...
			lf_obj_handle_t * ohandles, void * fdatap)
{
  ...
  err = lf_write_attr(fhandle, ohandle, RGB_IMAGE, tmp_img->nbytes, (char *)tmp_img);
  ASSERTX(pass = 0, !err);

  gray_img = get_gray_ipl_image(tmp_img);

  err = lf_write_attr(fhandle, ohandle, GRAY_IPL_IMAGE, gray_img->nSize, (char *)gray_img);
  ASSERTX(pass = 0, !err);
  ...
  if(tmp_img) { lf_free_buffer(fhandle, (char *)tmp_img); }
  /* releasing the cvimage causes wierd things to happen if we read it back. 
   * pointers somewhere? */
  if(gray_img) { cvReleaseImageHeader(&gray_img); }
  ff_close(&file);
  return pass;
...
/* #endif */
/* 	lf_write_attr(fhandle, ohandle, II_DATA, sizeof(char*), (char *)&sum); /\* XXX *\/ */
/* 	lf_write_attr(fhandle, ohandle, II_SQ_DATA, sizeof(char*), (char *)&sum2); /\* XXX *\/ */
